...
def main(inputs, output):
    ...
    avgDict = dict(positiveSubreddits.collect())
    broadcastAvg = sc.broadcast(avgDict)

    commentBySub = jsonSubreddits.map(lambda c: (c['subreddit'], c))
    relativeScores = commentBySub.map(lambda x: relative_score(x, broadcastAvg))
    
    sortedScores = relativeScores.sortBy(lambda x: x[0], ascending=False)
    sortedScores.saveAsTextFile(output)

def relative_score(commentBySub, avg):
    subreddit, comment = commentBySub
    avg = avg.value.get(subreddit, None)
    if avg == None:
        return
